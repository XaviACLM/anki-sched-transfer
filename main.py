...
parser = argparse.ArgumentParser(prog='scheduling-transfer',
...
fix_okurigana_snafus = lambda text: text.replace('と共にと共に', 'と共に') \
    .replace('係係らず', '係わらず') \
    .replace('多多', '多い')
remove_pissy_notes = lambda text: text.replace('(literally no example with you) ', '') \
    .replace('(e-stem) ', '')
...
def main(args):
    ...
    if source_name.endswith('.apkg'): source_name = source_name[:-5]
    destination_name = args.destination
    if destination_name.endswith('.apkg'): destination_name = destination_name[:-5]
    output_name = args.output or destination_name
    if not output_name.endswith('.apkg'): output_name += '.apkg'
    output_source_name = args.output_source or source_name
    if not output_source_name.endswith('.apkg'): output_source_name += '.apkg'
    ...
    with DeckWrangler(source_name, proceed_if_unzipped=True) as old_deck, \
        ...
        print(f"{n_overlap} (on each deck) of these cards have been matched to transfer scheduling data.")
        print("")
        ...
        if len(new_surplus) > 0:
            if len(new_surplus) > 1:
                ...
                print(f"There is 1 card in the destination deck that has no equivalent in the source deck:")
            print(new_surplus)
            print(
        ...
        if len(old_surplus) > 0:
            if len(old_surplus) > 1:
                ...
                print(f"There is 1 card in the source deck that has no equivalent in the destination deck:")
            print(old_surplus)
            ensure_missing_arg()
            print("")

        # find the match
        index_pairs = pd.merge(normalized_names_old.reset_index(name='name'),
                               normalized_names_new.reset_index(name='name'),
                               on='name', suffixes=('_old', '_new'))[['id_old', 'id_new']]

        # gather the data that we will use to update it and switch the index to the new deck
        to_update = old_sched_data.merge(index_pairs, left_index=True, right_on="id_old", how="inner")
        to_update.drop("id_old", axis=1, inplace=True)
        to_update.set_index("id_new", inplace=True)
        to_update = to_update[["due", "ivl", "factor", "lapses", "type", "queue", "reps"]]
        ...
        new_sched_data.to_sql(name="cards", con=new_deck.engine, if_exists="replace", index=False)
        ...
        if args.missing == 'zero-out':
            ...
            old_sched_data.to_sql(name="cards", con=old_deck.engine, if_exists="replace", index=False)
            old_deck.commit(with_name=output_source_name, overwrite=True)

        # mo-mo doesn't seem like it has any actual difference between the two, really... !!!
        # both relative clauses are identical !!!

        # annotate, though, that ようと思う has no proper examples

        # writeup 1, writeup 2

        # explain why no furigana in name

        # look at description, if it specifies gender then use that gender

        # sometimes _ only one sentence but all audios play 
